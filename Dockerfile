FROM node:24.18.0-alpine3.23 AS build-nodes
...
FROM node:24.18.0-alpine3.23 AS build-hooks
...
FROM node:24.18.0-alpine3.23 AS build-ui
...
RUN cd /community-nodes && npm install --omit=dev
