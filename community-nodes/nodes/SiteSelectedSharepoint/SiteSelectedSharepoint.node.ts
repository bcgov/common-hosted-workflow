...
interface SiteSelectedSharepointCredentials {
  tenantId: string;
  clientId: string;
  graphBaseUrl: string;
...
export class SiteSelectedSharepoint implements INodeType {
  description: INodeTypeDescription = {
    displayName: 'Site Selected SharePoint',
    name: 'siteSelectedSharepoint',
    ...
    usableAsTool: true,
    defaults: { name: 'Site Selected SharePoint' },
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    const credentials = (await this.getCredentials(
      GRAPH_CREDENTIAL_TYPE,
    )) as unknown as SiteSelectedSharepointCredentials;
...
async function executeItem(
  ...
  credentials: SiteSelectedSharepointCredentials,
  retry: RetryOptions,
