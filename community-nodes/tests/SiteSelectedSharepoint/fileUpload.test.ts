...
import { uploadFile } from '../../nodes/SiteSelectedSharepoint/actions/file/upload';
...
describe('uploadFile — small file (<=4MB)', () => {
  it('PUTs directly to the content endpoint with conflictBehavior in the URL', async () => {
    ...
    const result = await uploadFile(
      ...
      { maxRetries: 1 },
      { siteId: 'site-1', driveId: 'drive-1', folderPath: 'Reports', fileName: 'file.pdf' },
      buffer,
      SMALL_OPTIONS,
    );
  ...
  it('percent-encodes special characters in folder/file names and preserves the conflictBehavior query string', async () => {
    ...
    const result = await uploadFile(
      ...
      'https://graph.microsoft.com/v1.0',
      { maxRetries: 1 },
      { siteId: 'site-1', driveId: 'drive-1', folderPath: 'Report #1', fileName: 'file #1.pdf' },
      buffer,
      SMALL_OPTIONS,
    );
  ...
  it('rejects a chunk size that is not a multiple of 320 KiB', async () => {
    ...
    await expect(
      uploadFile(
        ...
        'https://graph.microsoft.com/v1.0',
        { maxRetries: 1 },
        { siteId: 'site-1', driveId: 'drive-1', folderPath: '', fileName: 'file.pdf' },
        Buffer.from('x'),
        {
          ...SMALL_OPTIONS,
          chunkSizeBytes: 1000,
...
describe('uploadFile — large file (>4MB, chunked)', () => {
  it('creates an upload session then PUTs a chunk with the correct Content-Range', async () => {
    ...
    const result = await uploadFile(
      ...
      'https://graph.microsoft.com/v1.0',
      { maxRetries: 1 },
      { siteId: 'site-1', driveId: 'drive-1', folderPath: '', fileName: 'big.bin' },
      buffer,
  ...
  it('splits a file into multiple sequential chunks with correct offsets', async () => {
    ...
    const result = await uploadFile(
      ...
      'https://graph.microsoft.com/v1.0',
      { maxRetries: 1 },
      { siteId: 'site-1', driveId: 'drive-1', folderPath: '', fileName: 'multi.bin' },
      buffer,
