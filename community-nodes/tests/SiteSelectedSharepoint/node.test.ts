...
import { SiteSelectedSharepoint } from '../../nodes/SiteSelectedSharepoint/SiteSelectedSharepoint.node';

describe('SiteSelectedSharepoint node', () => {
  const node = new SiteSelectedSharepoint();

  it('has the expected identity', () => {
    expect(node.description.displayName).toBe('Site Selected SharePoint');
    expect(node.description.name).toBe('siteSelectedSharepoint');
    expect(node.description.version).toBe(1);
    expect(node.description.usableAsTool).toBe(true);
  });

  it('declares the siteSelectedSharepointOAuth2Api credential as required', () => {
    expect(node.description.credentials).toEqual([{ name: 'siteSelectedSharepointOAuth2Api', required: true }]);
  });
...
describe('SiteSelectedSharepoint execute() — Item Create wiring', () => {
...
describe('SiteSelectedSharepoint node — File + List resources', () => {
...
describe('SiteSelectedSharepoint execute() — File Download wiring', () => {
