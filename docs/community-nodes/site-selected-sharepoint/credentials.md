...
The Site Selected SharePoint node uses the **Site Selected SharePoint OAuth2 API** credential type (`siteSelectedSharepointOAuth2Api`). It extends n8n's standard OAuth2 credential and fixes the grant type to **Client Credentials** so n8n signs every Graph request and refreshes tokens automatically.
...
2. Search for **"Site Selected SharePoint OAuth2 API"** and select it
