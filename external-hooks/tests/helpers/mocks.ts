...
export function createMockN8nRepositories() {
  ...
  return {
    ...
    withTransaction: vi.fn(async (_manager: any, _context: any, handler: (em: any) => Promise<unknown>) => {
      const em = {
        create: vi.fn((_entityName: string, payload: Record<string, unknown>) => payload),
