 ...
 *  - group type      → any of the user's tenant groups must appear in the list
 *  - user type       → matches the user's email
 *  - comma-separated → any single match is sufficient
 */
export function canUserSeeTrigger(
  trigger: Trigger,
  ...
  if (allowedActorsType === 'group') return userGroups.some((g) => actors.has(g.toLowerCase()));
  if (allowedActorsType === 'user') return actors.has(userEmail.toLowerCase());

  // 'other' — no client-side resolution
  return false;
}
